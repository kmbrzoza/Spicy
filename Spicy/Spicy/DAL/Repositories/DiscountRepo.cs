...
namespace Spicy.DAL.Repositories
{
    ...
    {
        #region QUERIES
        private const string GET_ALL_DISCOUNTS = @"SELECT d.name, d.description, d.discount_code, d.start_date, d.end_date, d.curr_price, d.prev_price, u.name, c.name
                                                    FROM discount d
        ...
        private const string GET_DISCOUNTS_BY_CATEGORY = @"SELECT d.name, d.description, d.discount_code, d.start_date, d.end_date, d.curr_price, d.prev_price, u.name, c.name
                                                    FROM discount d
                                                    INNER JOIN publishedby p ON p.id_discount= d.id_discount
                                                    INNER JOIN user u ON p.id_user= u.id_user
                                                    INNER JOIN isin i ON i.id_discount= d.id_discount
